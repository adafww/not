  ...
  .friends-block
    ...
    .friends-block__actions
      ...
      template(v-else)
        ...
        .friends-block__actions-block.delete(v-tooltip.bottom="'Отклонить запрос'" @click="openModal('deleteRequest')" v-if="friend")
          simple-svg(:filepath="'/static/img/delete.svg'")
        ...
        .friends-block__actions-block(v-tooltip.bottom="'Добавить в друзья'" @click="apiAddFriends(info.id)")
          simple-svg(:filepath="'/static/img/friend-add.svg'")
  ...
  computed: {
    ...
    modalText() {
      return this.modalType === 'delete'
        ...
        : this.modalType === 'deleteRequest'
        ? `Вы уверены, что хотите отклонить запрос пользователя ${this.info.first_name + ' ' + this.info.last_name}?`
        : `Вы уверены, что хотите заблокировать пользователя ${this.info.first_name + ' ' + this.info.last_name}?`
    }
  },
  methods: {
    ...mapActions('profile/friends', ['apiAddFriends', 'apiDeleteFriends', 'apiDeleteFriendsRequest']),
    ...
    onConfrim(id) {
      this.modalType === 'delete'
        ...
        : this.modalType === 'deleteRequest'
        ? this.apiDeleteFriendsRequest(id).then(() => this.closeModal())
        : this.modalType === 'deleteModerator'
